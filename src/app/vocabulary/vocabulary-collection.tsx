...
export function VocabularyCollection({ vocabulary, userLevels }: VocabularyCollectionProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [filteredItems, setFilteredItems] = useState<VocabularyItem[]>(vocabulary);
  const [currentTab, setCurrentTab] = useState('all');
  const [wanikaniLevel, setWanikaniLevel] = useState(userLevels.wanikaniLevel);
  const [genkiChapter, setGenkiChapter] = useState(userLevels.genkiChapter);
  const [tadokuLevel, setTadokuLevel] = useState(userLevels.tadokuLevel);

  useEffect(() => {
    filterVocabulary();
  }, [searchQuery, currentTab, wanikaniLevel, genkiChapter, tadokuLevel]);
  ...
  if (!vocabulary.length) {
    return (
      <div className="text-center p-8 border rounded-lg bg-muted/10">
        <p className="text-lg">No vocabulary items available yet.</p>
      </div>
    );
  }
      ...
      <div className="mb-8">
        ...
        <Tabs defaultValue="all" onValueChange={setCurrentTab} className="w-full">
          <TabsList className="mb-4">
            ...
            <TabsTrigger value="wanikani">Wanikani</TabsTrigger>
            <TabsTrigger value="genki">Genki</TabsTrigger>
          </TabsList>
          
          <TabsContent value="all">
            <p className="text-sm text-muted-foreground mb-4">
              Showing all vocabulary items ({filteredItems.length})
            </p>
          </TabsContent>
          
          <TabsContent value="wanikani">
            <div className="mb-6">
              <p className="text-sm font-medium mb-2">Wanikani Level: {wanikaniLevel}</p>
              <Slider
                value={[wanikaniLevel]}
                onValueChange={(value) => setWanikaniLevel(value[0])}
                max={60}
                step={1}
                className="max-w-md"
              />
        ...
        {filteredItems.map((item, index) => (
          <Card key={index} className="flex flex-col h-full">
            ...
            <CardContent className="flex-grow">
              <p className="text-lg mb-4">{item.meaning}</p>
              <div className="mt-2">
