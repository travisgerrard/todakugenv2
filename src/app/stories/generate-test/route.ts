...
function logToFile(message: string) {
  ...
  try {
    ...
    const logPath = path.join(logDir, 'story-fallback.log');
...
console.log('📄 Initializing /stories/generate-test route (FALLBACK)');
logToFile('[INIT] Initializing /stories/generate-test route (FALLBACK)');
...
export async function GET(request: Request) {
  ...
  const logPrefix = `[StoryFallback-${requestId}]`;
  
  console.log(`${logPrefix} GET request received for fallback endpoint`);
  logToFile(`${logPrefix} GET request received for fallback endpoint`);
  
  return new NextResponse(
    JSON.stringify({ 
      status: 'online',
      message: 'The fallback story generation API is available. Use POST to generate stories.',
      path: '/stories/generate-test',
...
export async function OPTIONS(request: Request) {
  console.log('OPTIONS request received for fallback endpoint');
  logToFile('OPTIONS request received for fallback endpoint');
...
export async function POST(request: Request) {
  ...
  console.log(`${logPrefix} Fallback story API called`);
  logToFile(`${logPrefix} Fallback story API called`);
  ...
  try {
    ...
    try {
      ...
      const responseData = {
        success: true,
        message: 'Story generated successfully (via fallback API)',
        story: {
    ...
    console.error(`${logPrefix} Unhandled error in fallback story API route:`, error);
    logToFile(`${logPrefix} Unhandled error in fallback story API route: ${error instanceof Error ? error.message : 'Unknown error'}`);
